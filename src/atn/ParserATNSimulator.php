...
class ParserATNSimulator extends ATNSimulator
{
    ...
    static $debug_list_atn_decisions = false;
    ...
    function adaptivePredict(TokenStream $input, $decision, $outerContext)
    {
        ...
        {
            Logger::log("adaptivePredict decision " . $decision . " exec LA(1)==" . $this->getLookaheadName($input) . " line " . $input->LT(1)->line . ":" . $input->LT(1)->charPositionInLine);
        }
        ...
        try
        {
            ...
            if ($s0 === null)
            {
                ...
                if (self::$debug || self::$debug_list_atn_decisions)
                {
                    Logger::log("predictATN decision " . $dfa->decision . " exec LA(1)==" . $this->getLookaheadName($input) . ", outerContext=" . $outerContext->toString($this->parser->getRuleNames()));
                }
            ...
            if (self::$debug)
            {
                Logger::log("DFA after predictATN: " . $dfa->toString($this->parser->getVocabulary()));
            }
            return $alt;
        }
        finally
    ...
    function execATN($dfa, $s0, TokenStream $input, $startIndex, $outerContext )
    {
        if (self::$debug || self::$debug_list_atn_decisions)
        {
            Logger::log("execATN decision " . $dfa->decision . " exec LA(1)==" . $this->getLookaheadName($input) . " line " . $input->LT(1)->line . ":" . $input->LT(1)->charPositionInLine);
        }
        $previousD = $s0;
        ...
        while(true)
        {
            ...
            if ($D->requiresFullContext && $this->predictionMode !== PredictionMode::SLL)
            {
                ...
                if ($D->predicates !== null)
                {
                    if (self::$debug)
                    {
                        Logger::log("DFA state has preds in DFA sim LL failover");
                    }
                ...
                if (self::$dfa_debug)
                {
                    Logger::log("ctx sensitive state " . $outerContext . " in " . $D);
                }
                $fullCtx = true;
    ...
    // returns {@link //ERROR}.
    function computeTargetState($dfa, $previousD, $t)
    {
        ...
        $D = new DFAState(null, $reach);

        $predictedAlt = $this->getUniqueAlt($reach);

        if (self::$debug)
        {
            $altSubSets = PredictionMode::getConflictingAltSubsets($reach);
            Logger::log("SLL altSubSets=" . Utils::arrayToString($altSubSets) .
                        ", previous=" . $previousD->configs .
                        ", configs=" . $reach .
                        ", predict=" . $predictedAlt .
                        ", allSubsetsConflict=" . PredictionMode::allSubsetsConflict($altSubSets) .
                        ", conflictingAlts=" . $this->getConflictingAlts($reach));
        }
    ...
    function predicateDFAState(DFAState $dfaState, DecisionState $decisionState) : void
    {
        // We need to test all predicates, even in DFA states that
        // uniquely predict alternative.
        $nalts = count($decisionState->transitions);
    ...
    function execATNWithFullContext($dfa, $D,// how far we got before failing over
        ...
        if (self::$debug || self::$debug_list_atn_decisions)
        {
            Logger::log("execATNWithFullContext " . $s0);
        }
        $fullCtx = true;
        ...
        while (true)
        {
            ...
            if (self::$debug)
            {
                Logger::log("LL altSubSets=" . $altSubSets . ", predict=" .
    ...
    function computeReachSet(ATNConfigSet $closure, int $t, bool $fullCtx) : ?ATNConfigSet
    {
        if (self::$debug)
        {
            Logger::log("in computeReachSet, starting closure: " . $closure);
        }
        ...
        foreach ($closure->items() as $c)
        {
            if(self::$debug_add)
            {
                Logger::log("testing " . $this->getTokenName($t) . " at " . $c);
            }
            ...
            if ($c->state instanceof RuleStopState)
            {
                if ($fullCtx || $t === Token::EOF)
                {
                    ...
                    if (self::$debug_add)
                    {
                        Logger::log("added " . $c . " to skippedStopStates");
                    }
                }
                continue;
            }

            foreach ($c->state->transitions as $trans)
            {
                $target = $this->getReachableTarget($trans, $t);
                if ($target !== null)
                {
                    $cfg = new ATNConfig((object)['state'=>$target], $c);
                    $intermediate->add($cfg, $this->mergeCache);

                    if (self::$debug_add)
                    {
                        Logger::log("added " . $cfg . " to intermediate");
                    }
                }
            }
        }
        // Now figure out where the reach operation can take us...
        /** @var ATNConfigSet $reach */
        $reach = null;
    ...
    function removeAllConfigsNotInRuleStopState(ATNConfigSet $configs, bool $lookToEndOfRule) : ATNConfigSet
    {
        ...
        foreach ($configs->items() as $config)
        {
            ...
            if ($lookToEndOfRule && $config->state->onlyHasEpsilonTransitions())
            {
                ...
                if ($nextTokens->contains(Token::EPSILON))
                {
                    ...
                    $result->add(new ATNConfig((object)[ 'state'=>$endOfRuleState ], $config), $this->mergeCache);
                }
            }
        }
    ...
    function computeStartState(ATNState $p, RuleContext $ctx, bool $fullCtx) : ATNConfigSet
    {
        ...
        {
            $c = new ATNConfig((object)[ 'state' => $t->target, 'alt' => $i + 1, 'context' => $initialContext ], null);
            $closureBusy = new Set();
    ...
    function applyPrecedenceFilter(ATNConfigSet $configs) : ATNConfigSet
    {
        ...
        foreach ($configs->items() as $config)
        {
            ...
            {
                $configSet->add(new ATNConfig((object)[ 'semanticContext'=>$updatedContext ], $config), $this->mergeCache);
            }
            else
            {
    ...
    function getAltThatFinishedDecisionEntryRule(ATNConfigSet $configs)
    {
        ...
        return count($alts) === 0 ? ATN::INVALID_ALT_NUMBER : min($alts);
    }
    ...
    /**
     * @param $configs ATNConfigSet
     * @param $outerContext ParserRuleContext
     * @return ATNConfigSet[]
     * @throws \Exception
     */
    function splitAccordingToSemanticValidity(ATNConfigSet $configs, ParserRuleContext $outerContext) : array
    {
    ...
    protected function evalSemanticContextMany(array $predPredictions, ParserRuleContext $outerContext, bool $complete) : BitSet
    {
        ...
        foreach ($predPredictions as $pair)
        {
            ...
            {
                Logger::log("eval pred $pair = " . $predicateEvaluationResult);
            }
            if ($predicateEvaluationResult)
            {
                if (self::$debug || self::$dfa_debug)
                {
                    Logger::log("PREDICT " . $pair->alt);
                }
                $predictions->add($pair->alt);
                if (!$complete) break;
            }
    ...
    function closure(ATNConfig $config, ATNConfigSet $configs, Set $closureBusy, bool $collectPredicates, bool $fullCtx, bool $treatEofAsEpsilon) : void
    {
        $initialDepth = 0;
        $this->closureCheckingStopState($config, $configs, $closureBusy, $collectPredicates, $fullCtx, $initialDepth, $treatEofAsEpsilon);
    }

    function closureCheckingStopState(ATNConfig $config, ATNConfigSet $configs, Set $closureBusy, bool $collectPredicates, bool $fullCtx, int $depth, bool $treatEofAsEpsilon) : void
    {
        if (self::$debug || self::$debug_closure)
        {
            Logger::log("closure(" . $config->toString($this->parser, true) . ")");
                ...
                for ($i =0; $i < $config->context->getLength(); $i++)
                {
                    if ($config->context->getReturnState($i) === PredictionContext::EMPTY_RETURN_STATE)
                    {
                        ...
                        else
                        {
                            ...
                            if (self::$debug)
                            {
                                Logger::log("FALLING off rule " . $this->getRuleName($config->state->ruleIndex));
                            }
    ...
    function closure_(ATNConfig $config, ATNConfigSet $configs, Set $closureBusy, bool $collectPredicates, bool $fullCtx, int $depth, bool $treatEofAsEpsilon) : void
    {
        $p = $config->state;
        ...
        foreach ($p->transitions as $i => $t)
        {
            ...
            if ($c)
            {
                ...
                if ($config->state instanceof RuleStopState)
                {
                    ...
                    if (self::$debug)
                    {
                        Logger::log("dips into outer ctx: " . $c);
                    }
                }
                else
                {
    ...
    function canDropLoopEntryEdgeInLeftRecursiveRule(ATNConfig $config) : bool
    {
        // return False
        $p = $config->state;
        ...
        for ($i=0; $i<$numCtxs; $i++)
        {
            ...
            if ($returnStateTarget->stateType === ATNState::BLOCK_END
                && count($returnStateTarget->transitions) === 1
    ...
    function actionTransition(ATNConfig $config, ActionTransition $t) : ATNConfig
    {
        if (self::$debug)
        {
            Logger::log("ACTION edge " . $t->ruleIndex . ":" . $t->actionIndex);
        }
        return new ATNConfig((object)[ 'state'=>$t->target ], $config);
    }
    ...
    function precedenceTransition(ATNConfig $config, PrecedencePredicateTransition $pt, bool $collectPredicates, bool $inContext, bool $fullCtx) : ATNConfig
    {
        if (self::$debug)
        {
            Logger::log("PRED (collectPredicates=" . $collectPredicates . ") " . $pt->precedence . ">=_p, ctx dependent=true");
            if ($this->parser!==null)
            {
        ...
        if ($collectPredicates && $inContext)
        {
            if ($fullCtx)
            {
                ...
                if ($predSucceeds)
                {
                    $c = new ATNConfig((object)[ 'state'=>$pt->target ], $config);// no pred context
                }
            }
            else
            {
                $newSemCtx = SemanticContext::andContext($config->semanticContext, $pt->getPredicate());
                $c = new ATNConfig((object)[ 'state'=>$pt->target, 'semanticContext'=>$newSemCtx ], $config);
            }
        }
        else
        {
            $c = new ATNConfig((object)[ 'state'=>$pt->target ], $config);
        }
        if (self::$debug)
        {
            Logger::log("config from pred transition=" . $c);
        }
        return $c;
    }
    ...
    function ruleTransition(ATNConfig $config, $t) : ATNConfig
    {
        if (self::$debug)
        {
            Logger::log("CALL rule " . $this->getRuleName($t->target->ruleIndex) . ", ctx=" . $config->context);
        }
        $returnState = $t->followState;
        $newContext = SingletonPredictionContext::create($config->context, $returnState->stateNumber);
        return new ATNConfig((object)[ 'state'=>$t->target, 'context'=>$newContext ], $config);
    }

    function getConflictingAlts(ATNConfigSet $configs) : BitSet
    {
    ...
    function getTokenName(int $t) : string
    {
        ...
        $vocabulary = $this->parser ? $this->parser->getVocabulary() : VocabularyImpl::EMPTY_VOCABULARY();
		$displayName = $vocabulary->getDisplayName($t);
		if ($displayName === (string)$t) return $displayName;

		return "$displayName<$t>";
    }

    function getLookaheadName(TokenStream $input) : string
    {
    ...
    //  "dead" code for a bit.
    function dumpDeadEndConfigs(NoViableAltException $nvae) : void
    {
		...
		foreach ($nvae->deadEndConfigs->items() as $c)
		{
			/** @var string $trans */
			$trans = "no edges";
			if (count($c->state->transitions) > 0)
			{
				/** @var Transition $t */
    ...
    function noViableAlt(TokenStream $input, $outerContext, ATNConfigSet $configs, int $startIndex) : NoViableAltException
    {
        return new NoViableAltException($this->parser, $input, $input->get($startIndex), $input->LT(1), $configs, $outerContext);
    }

    function getUniqueAlt(ATNConfigSet $configs)
    {
    ...
    // on {@code to}
    function addDFAEdge(DFA $dfa, ?DFAState $from, int $t, ?DFAState $to)
    {
        if( self::$debug)
        {
            Logger::log("EDGE $from -> $to upon " . $this->getTokenName($t));
        }

        if (!$to) return null;

        $to = $this->addDFAState($dfa, $to);// used existing if possible not incoming
        if (!$from || $t < -1 || $t > $this->atn->maxTokenType) return $to;

        if (!$from->edges === null) $from->edges = [];

        $from->edges[$t + 1] = $to;

        if (self::$debug)
        {
            Logger::log("DFA =\n" . $dfa->toString($this->parser ? $this->parser->getVocabulary() : null));
        }
        return $to;
    }
    ...
    function addDFAState(DFA $dfa, DFAState $D) : DFAState
    {
        ...
        if (self::$debug)
        {
            Logger::log("adding new DFA state: $D");
        }
        return $D;
    }
    ...
    function reportContextSensitivity(DFA $dfa, int $prediction, ATNConfigSet $configs, int $startIndex, int $stopIndex) : void
    {
        if (self::$debug || self::$retry_debug)
        {
            $interval = new Interval($startIndex, $stopIndex);
